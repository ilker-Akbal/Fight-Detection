...
class PoseInferenceTimeout(PoseInferenceError):
    pass


def should_request_pose_inference(
    pose_enabled: bool,
    roi_available: bool,
    frame_idx: int,
    pose_stride: int,
    ...
    return (
        bool(pose_enabled)
        and bool(roi_available)
        and int(frame_idx) % max(1, int(pose_stride)) == 0
    )
...
class PoseResultValidator:
    """Validates pose result identity without keeping camera temporal state."""
...
class PoseInferenceClient:
    """Synchronous, one-outstanding-request client for one camera process."""
    ...
    def __init__(
        ...
        *,
        ...
        self.validator = PoseResultValidator(self.camera_id, self.generation)
    ...
    def infer(self, roi, frame_idx: int):
        request_created = time.perf_counter()
        payload = payload_metadata(roi) if self.metrics_enabled else {}
        ...
        try:
            ...
            raise PoseInferenceTimeout("Pose inference request queue is full") from exc
        ...
        while True:
            if self.stop_event is not None and self.stop_event.is_set():
                raise PoseInferenceError("Pipeline is stopping")
            ...
            if remaining <= 0:
                ...
                raise PoseInferenceTimeout(
                    f"Pose inference timed out after {self.inference_timeout_sec:.3f}s"
                )
            ...
            if result.error:
                ...
                raise PoseInferenceError(result.error)
            return result.pose_result
...
def run_pose_inference_loop(
    ...
    result_timeout_sec = float(runtime.get("pose_result_enqueue_timeout_sec", 1.0))
    metrics_enabled, _, _ = metrics_runtime_config(runtime)
    warmup_requests = metrics_warmup_requests(runtime)
    batch_enabled = bool(runtime.get("pose_batch_enabled", False))
    batch_size_limit = max(1, int(runtime.get("pose_batch_size", 1)))
    batch_max_wait_ms = max(0.0, float(runtime.get("pose_batch_max_wait_ms", 0.0)))
    ...
    if adapter_factory is None:
        ...
        adapter_factory = PoseAdapter
    adapter = adapter_factory(models["pose_config"])
    ...
    _report(
        report_queue,
        {
            ...
            "weights": getattr(adapter, "weights", "configured_in_pose_yaml"),
    ...
    while True:
        ...
        try:
            ...
            for request, worker_received_monotonic, request_index in zip(
                ...
                if metrics_enabled:
                    if payload_bytes <= 0:
                        payload_bytes = payload_metadata(request.roi)["bytes"]
            ...
            outputs: list[tuple[object, str | None]] | None = None
            if batch_enabled and len(batch) > 1 and hasattr(adapter, "evaluate_batch"):
                try:
                    batch_outputs = list(
                        adapter.evaluate_batch([request.roi for request in batch])
                    )
                    if len(batch_outputs) != len(batch):
                        raise RuntimeError(
                            f"Pose batch output mismatch: requests={len(batch)} outputs={len(batch_outputs)}"
                        )
                    outputs = [(pose_result, None) for pose_result in batch_outputs]
                except Exception:
                    outputs = None
            if outputs is None:
                outputs = []
                for request in batch:
                    try:
                        outputs.append((adapter.evaluate(request.roi), None))
                    except Exception as exc:
                        outputs.append((None, f"{type(exc).__name__}: {exc}"))
                        errors += 1
...
def pose_inference_process_main(
    config: dict,
    request_queue,
    result_queue,
    report_queue,
    ...
    configure_process_runtime(
        cv2_threads=int(runtime.get("pose_worker_cv2_threads", runtime.get("cv2_threads", 1))),
